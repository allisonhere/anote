...
print_header() {
  clear
  echo -e "${BLUE}╔════════════════════════════════════════════════════════════╗${NC}"
  echo -e "${BLUE}║${NC}              ${BOLD}${CYAN}anote Release Builder${NC}                          ${BLUE}║${NC}"
  echo -e "${BLUE}╚════════════════════════════════════════════════════════════╝${NC}"
  echo ""
}
...
build_all() {
  ...
  local targets=(
    ...
    "x86_64-apple-darwin        anote-darwin-x86_64  darwin x86_64"
    "aarch64-apple-darwin       anote-darwin-aarch64 darwin arm64"
  )

  for entry in "${targets[@]}"; do
    read -r triple name target_os target_arch <<<"$entry"

    # Determine if this is the native target
    ...
    rustup target add "$triple" &>/dev/null || true
    ...
    local pid=$!
    spinner $pid "Building $name..."
    wait $pid || { print_error "Build failed for $triple"; continue; }

    # Strip binary
    if command -v strip &>/dev/null && [[ "$target_os" == "linux" ]]; then
      strip "$bin_path" 2>/dev/null || true
    fi

    # Package as tar.gz
    tar -czf "$archive" -C "$(dirname "$bin_path")" "$BINARY_NAME"
...
Or download a binary below and extract it manually." \
    --repo "$REPO"
